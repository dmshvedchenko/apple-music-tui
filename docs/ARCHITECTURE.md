...
The backend performs a playback state query immediately at startup, then polls at 500 ms; the worker clamps backend intervals to 250–1000 ms. It bulk-discovers playlist metadata, then reads the Music.app library in bounded 400-track property-array batches. A small fair priority scheduler keeps the single Apple Event executor serialized while selecting direct playback/seek/volume/shuffle/repeat commands before foreground playlist loading, polls/artwork, and background refresh work; a low-priority request is serviced after at most eight foreground requests. It cannot preempt an already-running Apple Event. Duplicate refresh, playlist-load, and artwork requests are coalesced. Opening an unloaded playlist emits `Loading` in the reducer immediately and fetches a 40-track first batch; subsequent 200-track batches preserve total-load throughput and remain ahead of the library scan. Completed playlist contents stay process-cached. Collection responses carry the current playback snapshot so Music.app remains authoritative throughout the scan. After the final library batch, deterministic grouping derives albums, artists, Recently Added, and local Recently Played. Cache serialization/atomic writes run outside the automation worker. Album identity combines normalized album artist, album title, and year; track order is disc number, track number, title, then stable ID. Application state maintains normalized search entries as collection batches arrive, then performs one final rebuild after derivation; keypresses filter this cache instead of rescanning raw Music.app objects. Results retain typed track, artist, album, or playlist identities and route through the normal detail/playback actions.
